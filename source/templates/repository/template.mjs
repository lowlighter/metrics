...
export default async function (
  ...
  if (!repo) {
    console.debug(
      `metrics/compute/${login}/${repo} > error, repo was undefined`
    );
    data.errors.push({
      error: {
        message: `You must pass a "repo" argument to use this template`,
      },
    });
    return await imports.plugins.core(...arguments);
  }
  console.debug(
    `metrics/compute/${login}/${repo} > switching to mode ${account}`
  );

  //Retrieving single repository
  console.debug(
    `metrics/compute/${login}/${repo} > retrieving single repository ${repo}`
  );
  const {
  ...
  data.repo.contributors = {
    totalCount: (
      await rest.repos.listContributors({ owner: data.repo.owner.login, repo })
    ).data.length,
  };
  ...
  for (let page = 0; page < 100; page++) {
    ...
    try {
      ...
      if (!data.length) {
        console.debug(
          `metrics/compute/${login}/${repo} > no more page to load`
        );
        break;
      }
      commits.push(...data);
  ...
  console.debug(
    `metrics/compute/${login}/${repo} > ${commits.length} commits loaded`
  );
  ...
  const contributions = commits.map(({ commit }) =>
    Math.abs(
      Math.ceil((now - new Date(commit.committer.date)) / (24 * 60 * 60 * 1000))
    )
  );
  ...
  //Override contributions calendar
  data.user.calendar.contributionCalendar.weeks = calendar.map((commit) => ({
    contributionDays: {
      color: commit
        ? `var(--color-calendar-graph-day-L${Math.ceil(
            commit / max / 0.25
          )}-bg)`
  ...
  if (data.plugins.projects)
    data.plugins.projects.list?.map(
      (project) =>
        (project.name = project.name.replace(`(${login}/${repo})`, "").trim())
    );
}
