...
export default async function({login, imports, data, rest, q, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    const response = [...await Promise.allSettled(repositories.map(({repo, owner}) => imports.repofilter(`${owner}/${repo}`, skipped) ? {} : rest.repos.getViews({owner, repo})))].filter(({status}) => status === "fulfilled").map(({value}) => value)
