...
export default async function({login, data, imports, rest, q, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    const repos = {}, weeks = {}
    const response = [...await Promise.allSettled(repositories.map(async ({repo, owner}) => imports.repofilter(`${owner}/${repo}`, skipped) ? {} : {handle: `${owner}/${repo}`, stats: (await rest.repos.getContributorsStats({owner, repo})).data}))].filter(({status}) => status === "fulfilled").map((
      {value},
    ) => value)
