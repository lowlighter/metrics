      ...
      try {
        do {
          ...
          const filtered = edges
            .flatMap(({node: {createdAt: created, reactions: {nodes: reactions}}}) => ({created: new Date(created), reactions: reactions.filter(({user = {}}) => !ignored.includes(user.login?.toLocaleLowerCase())).map(({content}) => content)}))
