...
export default async function({login, q, imports, data, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    //Load inputs
    let {user, region, sections, "limit.skills": _limit_skills, "ignored.skills": _ignored_skills, "limit.recent": _limit_recent} = imports.metadata.plugins.leetcode.inputs({data, account, q})
    const result = {user, sections, languages: [], skills: [], problems: {}, recent: []}

    const url = {
      china:"https://leetcode.cn/graphql/noj-go",
    ...
    //Languages stats
    {
      ...
      const {data: {data: {matchedUser: {languageProblemCount: languages}}}} = await imports.axios.post(url, {variables: {username: user}, query: queries.leetcode[`${region}.languages`]()})
    ...
    //Skills stats
    {
      ...
      const {data: {data: {matchedUser: {tagProblemCounts: skills}}}} = await imports.axios.post(url, {variables: {username: user}, query: queries.leetcode[`${region}.skills`]()})
    ...
    //Problems
    {
      ...
      const {data: {data: {allQuestionsCount: all, matchedUser: {submitStatsGlobal: {acSubmissionNum: submissions}}}}} = await imports.axios.post(url, {variables: {username: user}, query: queries.leetcode[`${region}.problems`]()})
    ...
    //Recent submissions
    {
      ...
      const {data: {data: {recentAcSubmissionList: submissions}}} = await imports.axios.post(url, {variables: {username: user, limit: _limit_recent}, query: queries.leetcode[`${region}.recent`]()})
