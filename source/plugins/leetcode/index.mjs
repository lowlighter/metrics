...
export default async function({login, q, imports, data, queries, account}, {enabled = false, extras = false} = {}) {
  ...
  try {
    ...
    //Load inputs
    let {user, sections, "limit.skills": _limit_skills, "limit.recent": _limit_recent, url} = imports.metadata.plugins.leetcode.inputs({data, account, q})
    ...
    //Languages stats
    {
      ...
      const {data: {data: {matchedUser: {languageProblemCount: languages}}}} = await imports.axios.post(`${url}/graphql`, {variables: {username: user}, query: queries.leetcode.languages()})
    ...
    //Skills stats
    {
      ...
      const {data: {data: {matchedUser: {tagProblemCounts: skills}}}} = await imports.axios.post(`${url}/graphql`, {variables: {username: user}, query: queries.leetcode.skills()})
    ...
    //Problems
    {
      ...
      const {data: {data: {allQuestionsCount: all, matchedUser: {submitStatsGlobal: {acSubmissionNum: submissions}}}}} = await imports.axios.post(`${url}/graphql`, {variables: {username: user}, query: queries.leetcode.problems()})
    ...
    //Recent submissions
    {
      ...
      const {data: {data: {recentAcSubmissionList: submissions}}} = await imports.axios.post(`${url}/graphql`, {variables: {username: user, limit: _limit_recent}, query: queries.leetcode.recent()})
