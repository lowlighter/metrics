...
export default async function (
  ...
  try {
    ...
    if (!enabled || !q.gists) return null;
    ...
    const gists = [];
    {
      ...
      do {
        console.debug(
          `metrics/compute/${login}/plugins > gists > retrieving gists after ${cursor}`
        );
        const {
          user: {
          ...
          queries.gists({ login, after: cursor ? `after: "${cursor}"` : "" })
        );
      ...
      console.debug(
        `metrics/compute/${login}/plugins > gists > loaded ${gists.length} gists`
      );
    }

    //Iterate through gists
    console.debug(
      `metrics/compute/${login}/plugins > gists > processing ${gists.length} gists`
    );
    let stargazers = 0,
      forks = 0,
      comments = 0,
