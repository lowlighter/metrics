...
export default async function (
  ...
  try {
    ...
    //Load inputs
    let { sections, days, limit } = imports.metadata.plugins.wakatime.inputs({
      data,
      account,
      q,
    });
    ...
    const {
      ...
      `https://wakatime.com/api/v1/users/current/stats/${range}?api_key=${token}`
    );
    const result = {
      sections,
      days,
      ...
      projects: stats.projects
        .map(({ name, percent, total_seconds: total }) => ({
          name,
          percent: percent / 100,
          total,
        }))
