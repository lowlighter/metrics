...
export default async function (
  ...
  try {
    ...
    if (!enabled || !q.tweets) return null;

    //Load inputs
    let { limit, user: username } = imports.metadata.plugins.tweets.inputs({
      data,
      account,
      q,
    });
    ...
    //Load user profile
    console.debug(
      `metrics/compute/${login}/plugins > tweets > loading twitter profile (@${username})`
    );
    const {
    ...
    if (profile?.profile_image_url) {
      console.debug(
        `metrics/compute/${login}/plugins > tweets > loading profile image`
      );
    ...
    const {
      ...
      `https://api.twitter.com/2/tweets/search/recent?query=from:${username}&tweet.fields=created_at&expansions=entities.mentions.username`,
      { headers: { Authorization: `Bearer ${token}` } }
    );
    ...
    if (limit > 0) {
      console.debug(
        `metrics/compute/${login}/plugins > tweets > keeping only ${limit} tweets`
      );
      tweets.splice(limit);
    }
    ...
    await Promise.all(
      tweets.map(async (tweet) => {
        //Mentions
        tweet.mentions =
          tweet.entities?.mentions.map(({ username }) => username) ?? [];
        //Format text
        console.debug(
          `metrics/compute/${login}/plugins > tweets > formatting tweet ${tweet.id}`
        );
        tweet.text = imports.htmlescape(
          //Escape tags
          imports
            .htmlescape(tweet.text, { "<": true, ">": true })
            //Mentions
            .replace(
              new RegExp(`@(${tweet.mentions.join("|")})`, "gi"),
