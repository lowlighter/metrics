...
export class Analyzer {
  ...
  constructor(
    login,
    {
      ...
      timeout = { global: NaN, repositories: NaN },
    }
  ) {
    //User informations
    this.login = login;
    ...
    this.parser =
      /^(?<login>[\s\S]+?)\/(?<name>[\s\S]+?)(?:@(?<branch>[\s\S]+?)(?::(?<ref>[\s\S]+))?)?$/;
    this.consumed = false;
    ...
    //Results
    this.results = {
      partial: { global: false, repositories: false },
      total: 0,
      lines: {},
      stats: {},
      colors: {},
      commits: 0,
      files: 0,
      missed: { lines: 0, bytes: 0, commits: 0 },
      elapsed: 0,
    };
  ...
  async run(runner) {
    if (this.consumed)
      throw new Error(
        "This analyzer has already been consumed, another instance needs to be created to perform a new analysis"
      );
    this.consumed = true;
      ...
      if (Number.isFinite(this.timeout.global)) {
        ...
        setTimeout(() => {
          if (!completed) {
            try {
              this.debug(
                `reached maximum execution time of ${this.timeout.global}m for analysis`
              );
  ...
  parse(repository) {
    ...
    if (typeof repository === "string") {
      ...
      const { login, name, ...groups } =
        repository.match(this.parser)?.groups ?? {};
  ...
  async analyze(path, { commits = [] } = {}) {
    ...
    if (this.timeout.repositories)
      this.debug(
        `timeout for repository analysis set to ${this.timeout.repositories}m`
      );
    for (const commit of commits) {
      ...
      if (this.timeout.repositories && elapsed > this.timeout.repositories) {
        ...
        this.debug(
          `reached maximum execution time of ${this.timeout.repositories}m for repository analysis (${elapsed}m elapsed)`
        );
        break;
      }
      try {
        const { total, files, missed, lines, stats } = await this.linguist(
          path,
          { commit, cache }
        );
        ...
        for (const language in lines) {
          if (this.categories.includes(cache.languages[language]?.type))
            this.results.lines[language] =
              (this.results.lines[language] ?? 0) + lines[language];
        }
        for (const language in stats) {
          if (this.categories.includes(cache.languages[language]?.type))
            this.results.stats[language] =
              (this.results.stats[language] ?? 0) + stats[language];
        }
      } catch (error) {
        ...
        if (processed % 50 === 0 || processed === commits.length)
          this.debug(
            `at commit ${processed}/${commits.length} (${(
              (100 * processed) /
              commits.length
            ).toFixed(2)}%, ${elapsed.toFixed(2)}m elapsed)`
          );
      }
    }
    this.results.colors = Object.fromEntries(
      Object.entries(cache.languages).map(([lang, { color }]) => [lang, color])
    );
  }
  ...
  ignore(repository) {
    ...
    if (ignored)
      this.debug(
        ...
        } as it matches skipped repositories`
      );
    return ignored;
  }
  ...
  debug(message) {
    return console.debug(
      `metrics/compute/${
        this.login
      }/plugins > languages > ${this.constructor.name
        .replace(/([a-z])([A-Z])/, (_, a, b) => `${a} ${b.toLocaleLowerCase()}`)
