    ...
    const patches = [
      ...await Promise.allSettled(
        commits
          ...
          .filter(commit => commit?.author && data.shared["commits.authoring"].some(authoring =>
            commit.author.login?.toLocaleLowerCase().includes(authoring) ||
            commit.author.email?.toLocaleLowerCase().includes(authoring)
          ))
