...
export default async function (
  { login, graphql, data, imports, q, queries, account },
  { enabled = false } = {}
) {
  //Plugin execution
  try {
    //Check if plugin is enabled and requirements are met
    if (!enabled || !q.stargazers) return null;
    ...
    console.debug(
      `metrics/compute/${login}/plugins > stargazers > querying api`
    );
    const repositories =
      data.user.repositories.nodes.map(
        ({ name: repository, owner: { login: owner } }) => ({
          repository,
          owner,
        })
      ) ?? [];
    ...
    for (const { repository, owner } of repositories) {
      ...
      console.debug(
        `metrics/compute/${login}/plugins > stargazers > retrieving stargazers of ${repository}`
      );
      let cursor = null;
      let pushed = 0;
      do {
        console.debug(
          `metrics/compute/${login}/plugins > stargazers > retrieving stargazers of ${repository} after ${cursor}`
        );
        const {
          repository: {
            stargazers: { edges },
      ...
      //Limit repositories
      console.debug(
        `metrics/compute/${login}/plugins > stargazers > loaded ${dates.length} stargazers for ${repository}`
      );
    }
    console.debug(
      `metrics/compute/${login}/plugins > stargazers > loaded ${dates.length} stargazers in total`
    );
