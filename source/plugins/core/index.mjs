...
export default async function (
  { login, q, dflags },
  { conf, data, rest, graphql, plugins, queries, account },
  { pending, imports }
) {
  //Load inputs
  ...
  if (q["config.timezone"]) {
    const timezone = (data.config.timezone = {
      name: q["config.timezone"],
      offset: 0,
    });
    try {
      timezone.offset =
        Number(
          new Date()
            ...
            .match(/UTC[+](?<offset>\d+)/)?.groups?.offset *
            60 *
            60 *
            1000
        ) || 0;
      console.debug(
        `metrics/compute/${login} > timezone set to ${timezone.name} (${
          timezone.offset > 0 ? "+" : ""
        }${Math.round(timezone.offset / (60 * 60 * 1000))} hours)`
      );
    } catch {
      timezone.error = `Failed to use timezone "${timezone.name}"`;
      console.debug(
        `metrics/compute/${login} > failed to use timezone "${timezone.name}"`
      );
    }
  }
  ...
  if ("config.animations" in q) {
    ...
    console.debug(
      `metrics/compute/${login} > animations ${
        data.animated ? "enabled" : "disabled"
      }`
    );
  }
  ...
  for (const name of Object.keys(imports.plugins)) {
    if (!plugins[name]?.enabled) continue;
    pending.push(
      (async () => {
        try {
          ...
          console.debug(
            `metrics/compute/${login}/plugins > ${name} > completed`
          );
        } catch (error) {
          console.debug(
            `metrics/compute/${login}/plugins > ${name} > completed (error)`
          );
          data.plugins[name] = error;
        } finally {
          const result = { name, result: data.plugins[name] };
          console.debug(
            imports.util.inspect(result, {
              depth: Infinity,
              maxStringLength: 256,
            })
          );
  ...
  for (const repository of data.user.repositories.nodes) {
    ...
    if (repository.licenseInfo)
      computed.licenses.used[repository.licenseInfo.spdxId] =
        (computed.licenses.used[repository.licenseInfo.spdxId] ?? 0) + 1;
  }

  //Total disk usage
  computed.diskUsage = `${imports.bytes(
    data.user.repositories.totalDiskUsage * 1000
  )}`;
  ...
  computed.registration = years
    ? `${years} year${imports.s(years)} ago`
    : months
    ? `${months} month${imports.s(months)} ago`
    : `${Math.ceil(diff * 365)} day${imports.s(Math.ceil(diff * 365))} ago`;
  computed.cakeday =
    years > 1
      ? [new Date(), new Date(data.user.createdAt)]
          .map(
            (date) =>
              date.toISOString().match(/(?<mmdd>\d{2}-\d{2})(?=T)/)?.groups
                ?.mmdd
          )
          .every((v, _, a) => v === a[0])
      : false;
  ...
  //Compute calendar
  computed.calendar = data.user.calendar.contributionCalendar.weeks
  ...
  //Token scopes
  computed.token.scopes = conf.settings.notoken
    ? []
    : (await rest.request("HEAD /")).headers["x-oauth-scopes"].split(", ");
    ...
    pending.push(
      (async () => {
        ...
        if (data.plugins.isocalendar?.svg)
          data.plugins.isocalendar.svg = halloween(
            data.plugins.isocalendar.svg
          );
