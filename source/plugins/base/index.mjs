...
export default async function (
  { login, graphql, data, q, queries, imports },
  conf
) {
  //Load inputs
  ...
  let {
    repositories,
    repositories_forks: forks,
  } = imports.metadata.plugins.base.inputs(
    { data, q, account: "bypass" },
    { repositories: conf.settings.repositories ?? 100 }
  );

  //Skip initial data gathering if not needed
  if (conf.settings.notoken) return postprocess.skip({ login, data }), {};

  //Base parts (legacy handling for web instance)
  const defaulted = "base" in q ? legacy.converter(q.base) ?? true : true;
  for (const part of conf.settings.plugins.base.parts)
    data.base[part] =
      `base.${part}` in q ? legacy.converter(q[`base.${part}`]) : defaulted;
  ...
  for (const account of ["user", "organization"]) {
    try {
      ...
      const queried = await graphql(
        queries.base[account]({
          login,
          "calendar.from": new Date(
            Date.now() - 14 * 24 * 60 * 60 * 1000
          ).toISOString(),
          "calendar.to": new Date().toISOString(),
          forks: forks ? "" : ", isFork: false",
        })
      );
      ...
      //Query repositories from GitHub API
      {
        ...
        do {
          console.debug(
            `metrics/compute/${login}/base > retrieving repositories after ${cursor}`
          );
          const {
            [account]: {
            ...
            queries.base.repositories({
              ...
              repositories: Math.min(
                repositories,
                { user: 100, organization: 25 }[account]
              ),
              forks: forks ? "" : ", isFork: false",
            })
          );
          ...
          pushed &&
          cursor &&
          data.user.repositories.nodes.length < repositories
        );
        //Limit repositories
        console.debug(
          `metrics/compute/${login}/base > keeping only ${repositories} repositories`
        );
        data.user.repositories.nodes.splice(repositories);
        console.debug(
          `metrics/compute/${login}/base > loaded ${data.user.repositories.nodes.length} repositories`
        );
      }
      //Success
      console.debug(
        `metrics/compute/${login}/base > graphql query > account ${account} > success`
      );
      return {};
    } catch (error) {
      console.debug(
        `metrics/compute/${login}/base > account ${account} > failed : ${error}`
      );
