    ...
    const activity = (await Promise.all(
      events
        ...
        .map(async ({type, payload, actor: {login: actor}, repo: {name: repo}, created_at}) => {
          ...
          if (imports.repofilter(repo, skipped))
            return null
          switch (type) {
            ...
            case "CommitCommentEvent": {
              ...
              if (ignored.includes(user?.toLocaleLowerCase()))
                return null
            ...
            case "PushEvent": {
              ...
              commits = commits.filter(({author: {email}}) => !ignored.includes(email?.toLocaleLowerCase()))
              if (!commits.length)
                return null
