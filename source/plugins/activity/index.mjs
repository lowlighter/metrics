...
export default async function (
  { login, data, rest, q, account, imports },
  { enabled = false } = {}
) {
  //Plugin execution
  try {
    //Check if plugin is enabled and requirements are met
    if (!enabled || !q.activity) return null;

    //Load inputs
    let { limit, days, filter } = imports.metadata.plugins.activity.inputs({
      data,
      q,
      account,
    });
    ...
    const { data: events } = await rest.activity.listEventsForAuthenticatedUser(
      {
        username: login,
        per_page: 100,
      }
    ...
    console.debug(
      `metrics/compute/${login}/plugins > activity > ${events.length} events loaded`
    );
        ...
        switch (type) {
          ...
          case "IssueCommentEvent": {
            ...
            const {
              ...
              comment: { body: content, performed_via_github_app: mobile },
            } = payload;
            return {
              type: "comment",
              on: "issue",
          ...
          case "MemberEvent": {
            if (!["added"].includes(payload.action)) return null;
            const {
              member: { login: user },
            } = payload;
          ...
          case "PullRequestEvent": {
            if (!["opened", "closed"].includes(payload.action)) return null;
            const {
              action,
              pull_request: {
                user: { login: user },
          ...
          case "PushEvent": {
            ...
            return {
              ...
              size,
              branch:
                ref.match(/refs.heads.(?<branch>.*)/)?.groups?.branch ?? null,
              commits: commits.map(({ sha, message }) => ({
          ...
          //Released
          case "ReleaseEvent": {
            if (!["published"].includes(payload.action)) return null;
            const {
              action,
              release: { name, prerelease, draft },
            } = payload;
            return { type: "release", repo, action, name, prerelease, draft };
          }
          //Starred a repository
          case "WatchEvent": {
            if (!["started"].includes(payload.action)) return null;
