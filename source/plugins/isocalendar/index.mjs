...
export default async function (
  { login, data, graphql, q, imports, queries, account },
  { enabled = false } = {}
) {
  //Plugin execution
  try {
    //Check if plugin is enabled and requirements are met
    if (!enabled || !q.isocalendar) return null;

    //Load inputs
    let { duration } = imports.metadata.plugins.isocalendar.inputs({
      data,
      account,
      q,
    });
    ...
    if (duration === "full-year") start.setFullYear(now.getFullYear() - 1);
    else start.setHours(-24 * 180);
    ...
    console.debug(
      `metrics/compute/${login}/plugins > isocalendar > querying api`
    );
    const calendar = {};
    for (const [name, from, to] of [
      ...
      console.debug(
        `metrics/compute/${login}/plugins > isocalendar > loading ${name} from "${from.toISOString()}" to "${to.toISOString()}"`
      );
      const {
        user: {
          calendar: {
    ...
    //Apply padding
    console.debug(
      `metrics/compute/${login}/plugins > isocalendar > applying padding`
    );
    const firstweek = calendar.weeks[0].contributionDays;
    const padded = calendar.padding
      .flatMap(({ contributionDays }) => contributionDays)
      .filter(({ date }) => !firstweek.map(({ date }) => date).includes(date));
    while (firstweek.length < 7) firstweek.unshift(padded.pop());
    ...
    let i = 0,
      j = 0;
    let svg = `
            <svg version="1.1" xmlns="http://www.w3.org/2000/svg" style="margin-top: -52px;" viewBox="0,0 480,${
                ...
                <filter id="brightness${k}">
                  <feComponentTransfer>
                    ${[..."RGB"]
                      .map(
                        (channel) =>
                          `<feFunc${channel} type="linear" slope="${
                            1 - k * 0.4
                          }" />`
                      )
                      .join("")}
    ...
    for (const week of calendar.weeks) {
      ...
      for (const day of week.contributionDays) {
        ...
        svg += `
                    <g transform="translate(${j * -1.7}, ${
          j + (1 - ratio) * size
        })">
                      <path fill="${day.color}" d="M1.7,2 0,1 1.7,0 3.4,1 z" />
                      <path fill="${
                        day.color
                      }" filter="url(#brightness1)" d="M0,1 1.7,2 1.7,${
          2 + ratio * size
        } 0,${1 + ratio * size} z" />
                      <path fill="${
                        day.color
                      }" filter="url(#brightness2)" d="M1.7,2 3.4,1 3.4,${
          1 + ratio * size
        } 1.7,${2 + ratio * size} z" />
                    </g>`;
        j++;
