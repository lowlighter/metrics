    ...
    //Plugin execution
      try {
        ...
        //Initialization
          ...
          const ranks = await graphql(queries.achievements.ranking({followers:user.followers.totalCount, created:user.repositories.totalCount, stars:user.popular.nodes?.[0]?.stargazers?.totalCount ?? 0, forks:Math.max(0, ...data.user.repositories.nodes.map(({forkCount}) => forkCount))}))
          await total({imports})
        ...
        //Developer
          {
            ...
            list.push({
              ...
              leaderboard:{
                user:1+ranks.created_rank.userCount,
                total:total.users,
                get top() { return Number(`1${"0".repeat(Math.ceil(Math.log10(this.user)))}`) },
                get percentile() { return 100*(this.user/this.top) },
                type:"users",
              }
            })
          }
        ...
        //Influencer
          {
            ...
            list.push({
              ...
              ...rank(value, [1, 200, 500, 1000]), value, unlock:new Date(unlock?.createdAt),
              leaderboard:{
                user:1+ranks.user_rank.userCount,
                total:total.users,
                get top() { return Number(`1${"0".repeat(Math.ceil(Math.log10(this.user)))}`) },
                get percentile() { return 100*(this.user/this.top) },
                type:"users",
        ...
        //Maintainer
          {
            ...
            list.push({
              ...
              leaderboard:{
                user:1+ranks.repo_rank.repositoryCount,
        ...
        //Inspirationer
          {
            ...
            list.push({
              ...
              leaderboard:{
                user:1+ranks.forks_rank.repositoryCount,
                total:total.repositories,
                get top() { return Number(`1${"0".repeat(Math.ceil(Math.log10(this.user)))}`) },
                get percentile() { return 100*(this.user/this.top) },
                type:"repositories",
