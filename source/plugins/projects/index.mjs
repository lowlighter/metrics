...
export default async function (
  { login, data, imports, graphql, q, queries, account },
  { enabled = false } = {}
) {
  //Plugin execution
  try {
    //Check if plugin is enabled and requirements are met
    if (!enabled || !q.projects) return null;
    ...
    let {
      ...
      repositories,
      descriptions,
    } = imports.metadata.plugins.projects.inputs({ data, account, q });
    //Repositories projects
    repositories = repositories.filter((repository) =>
      /[-\w]+[/][-\w]+[/]projects[/]\d+/.test(repository)
    );
    ...
    const {
      [account]: { projects },
    } = await graphql(queries.projects.user({ login, limit, account }));

    //Retrieve repositories projects from graphql api
    for (const identifier of repositories) {
      //Querying repository project
      console.debug(
        `metrics/compute/${login}/plugins > projects > querying api for ${identifier}`
      );
      const { user, repository, id } = identifier.match(
        /(?<user>[-\w]+)[/](?<repository>[-\w]+)[/]projects[/](?<id>\d+)/
      )?.groups;
      const {
        [account]: {
          repository: { project },
        ...
        queries.projects.repository({ user, repository, id, account })
      );
      //Adding it to projects list
      console.debug(
        `metrics/compute/${login}/plugins > projects > registering ${identifier}`
      );
    ...
    console.debug(
      `metrics/compute/${login}/plugins > projects > processing ${projects.nodes.length} projects`
    );
    const list = [];
    for (const project of projects.nodes) {
      ...
      if (time < 1) updated = "less than 1 day ago";
      else if (time < 30)
      ...
      list.push({
        ...
        progress: { enabled, todo, doing, done, total: todo + doing + done },
      });
    }

    //Limit
    console.debug(
      `metrics/compute/${login}/plugins > projects > keeping only ${limit} projects`
    );
    list.splice(limit);
