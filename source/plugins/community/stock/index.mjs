...
export default async function ({ login, q, imports, data, account }, { enabled = false, extras = false, token } = {}) {
	...
	try {
		...
		if (!q.stock || !imports.metadata.plugins.stock.enabled(enabled, { extras })) return null;

		//Load inputs
		let { symbol, interval, duration } = imports.metadata.plugins.stock.inputs({ data, account, q });
		if (!token) throw { error: { message: "API token is not set" } };
		if (!symbol) throw { error: { message: "Company stock symbol is not set" } };
		symbol = symbol.toLocaleUpperCase();

		//Query API for company informations
		console.debug(`metrics/compute/${login}/plugins > stock > querying api for company`);
		const {
			data: { quoteType: { shortName: company } = { shortName: symbol } },
		} = await imports.axios.get("https://yh-finance.p.rapidapi.com/stock/v2/get-profile", {
		...
		} = await imports.axios.get("https://yh-finance.p.rapidapi.com/stock/v2/get-chart", {
		...
		const chart = imports.Graph.timeline(
			close.map((y, i) => ({ x: new Date(timestamp[i] * 1000), y })),
			{ low: Math.min(...close), high: Math.max(...close), points: false, text: false, width: 480 * (1 + data.large), height: 200 }
		);
