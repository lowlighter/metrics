(async function () {
  ...
  return new Vue({
    ...
    data: {
      ...
      plugins: {
        ...
        descriptions: {
          ...
          ...Object.fromEntries(
            Object.entries(metadata).map(([key, { name }]) => [key, name])
          ),
        },
        options: {
          descriptions: {
            ...Object.assign(
              {},
              ...Object.entries(metadata).flatMap(([key, { web }]) => web)
            ),
          },
          ...Object.fromEntries(
            Object.entries(
    ...
    computed: {
      //User's avatar
      avatar() {
        return this.generated.content
          ? `https://github.com/${this.user}.png`
          : null;
      },
      //User's repository
      repo() {
      ...
      url() {
        ...
        //Template
        const template =
          this.templates.selected !== templates[0]
      ...
      action() {
        return [
          ...
          `          template: ${this.templates.selected}`,
          `          base: ${
            Object.entries(this.plugins.enabled.base)
              .filter(([key, value]) => value)
              .map(([key]) => key)
              .join(", ") || '""'
          }`,
          ...[
            ...Object.entries(this.plugins.enabled)
              .filter(([key, value]) => key !== "base" && value)
              .map(([key]) => `          plugin_${key}: yes`),
            ...Object.entries(this.plugins.options)
              .filter(([key, value]) => value)
              .filter(([key, value]) => this.plugins.enabled[key.split(".")[0]])
              .map(
                ([key, value]) =>
                  `          plugin_${key.replace(/[.]/, "_")}: ${
      ...
      configure() {
        ...
        const enabled = Object.entries(this.plugins.enabled)
          .filter(([key, value]) => value && key !== "base")
          .map(([key, value]) => key);
        const filter = new RegExp(`^(?:${enabled.join("|")})[.]`);
        //Search related options
        const entries = Object.entries(
          this.plugins.options.descriptions
        ).filter(([key, value]) => filter.test(key));
        entries.push(
          ...enabled.map((key) => [key, this.plugins.descriptions[key]])
        );
    ...
    methods: {
      ...
      mockresize() {
        ...
        if (svg) {
          const height =
            svg.querySelector("#metrics-end")?.getBoundingClientRect()?.y -
            svg.getBoundingClientRect()?.y;
          if (Number.isFinite(height)) svg.setAttribute("height", height);
        }
      },
      //Generate metrics and flush cache
      async generate() {
        //Avoid requests spamming
        if (this.generated.pending) return;
        this.generated.pending = true;
        //Compute metrics
        try {
          await axios.get(
            `/.uncache?&token=${
              (await axios.get(`/.uncache?user=${this.user}`)).data.token
            }`
          );
