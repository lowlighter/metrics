(function (
  ...
  async function load(url) {
    if (!cached.has(url)) cached.set(url, (await axios.get(url)).data);
  ...
  globalThis.placeholder = async function (set) {
    ...
    let { image, style, fonts, partials } = await load(
      `/.templates/${set.templates.selected}`
    );
    await Promise.all(
      partials.map(
        async (partial) =>
          await load(
            `/.templates/${set.templates.selected}/partials/${partial}.ejs`
          )
      )
    );
    //Trap includes
    image = image.replace(
      /<%-\s*await include[(](`.*?[.]ejs`)[)]\s*%>/g,
      (m, g) => `<%- await $include(${g}) %>`
    );
    //Faked data
    ...
    const data = {
      ...
      async $include(path) {
        const partial = await load(
          `/.templates/${set.templates.selected}/${path}`
        );
        return await ejs.render(partial, data, {
          async: true,
          rmWhitespace: true,
        });
      },
      ...
      computed: {
        ...
        cakeday: false,
        calendar: new Array(14).fill(null).map((_) => ({
        ...
        avatar:
          "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mOcOnfpfwAGfgLYttYINwAAAABJRU5ErkJggg==",
      },
      //User data
      account: "user",
      user: {
      ...
      plugins: {
        ...
        ...(set.plugins.enabled.tweets
          ? {
              tweets: {
                username:
                  options["tweets.user"] || "(attached Twitter account)",
                profile: {
                ...
                list: [
                  {
                    ...
                    text:
                      'Checkout metrics from  <span class="mention">@lowlighter</span>  !  <span class="hashtag">#GitHub</span> ',
                    mentions: ["lowlighter"],
                  },
                  ...new Array(Number(options["tweets.limit"]) - 1)
                    .fill(null)
                    .map((_) => ({
                      id: faker.random.number(100000000000000).toString(),
                      created_at: faker.date.recent(),
        ...
        ...(set.plugins.enabled.lines
          ? {
              lines: {
                ...
                deleted: `${faker.random.number(100)}.${faker.random.number(
                  9
                )}k`,
              },
            }
          ...
          ? {
              traffic: {
                views: {
                  ...
                  uniques: `${faker.random.number(10)}.${faker.random.number(
                    9
                  )}k`,
                },
              },
          ...
          ? {
              followup: {
                issues: {
                  ...
                  open: faker.random.number(1000),
                  closed: faker.random.number(1000),
                },
                pr: {
                  get count() {
                  ...
                  open: faker.random.number(1000),
                  merged: faker.random.number(1000),
                },
              },
            }
          : null),
        ...
        ...(set.plugins.enabled.languages
          ? {
              languages: {
                details: options["languages.details"]
                  .split(",")
                  .map((x) => x.trim())
                  .filter((x) => x),
                get colors() {
                  return Object.fromEntries(
                    Object.entries(this.favorites).map(([key, { color }]) => [
                      key,
                      color,
                    ])
                  );
                ...
                favorites: distribution(7).map((value, index, array) => ({
                  name: faker.lorem.word(),
                  color: faker.internet.color(),
                  value,
                  size: faker.random.number(1000000),
                  x: array.slice(0, index).reduce((a, b) => a + b, 0),
                })),
              },
            }
          : null),
        ...
        ...(set.plugins.enabled.people
          ? {
              get people() {
                const types = options["people.types"]
                  .split(",")
                  .map((x) => x.trim())
                  .map(
                    (x) =>
                      ({
                        ...
                        sponsoring: "sponsorshipsAsSponsor",
                      }[x] ?? x)
                  )
                  .filter((x) =>
                    [
                ...
                return {
                  ...
                  ...Object.fromEntries(
                    types.map((type) => [
                      type,
                      new Array(Number(options["people.limit"]))
                        .fill(null)
                        .map((_) => ({
        ...
        ...(set.plugins.enabled.music
          ? {
              music: {
                ...
                tracks: new Array(Number(options["music.limit"]))
                  .fill(null)
                  .map((_) => ({
        ...
        ...(set.plugins.enabled.pagespeed
          ? {
              pagespeed: {
                ...
                screenshot: options["pagespeed.screenshot"]
        ...
        ...(set.plugins.enabled.projects
          ? {
              projects: {
                ...
                list: new Array(Number(options["projects.limit"]))
                  .fill(null)
                  .map((_) => ({
                    ...
                    progress: {
                      enabled: true,
                      todo: faker.random.number(50),
                      doing: faker.random.number(50),
                      done: faker.random.number(50),
                      get total() {
        ...
        ...(set.plugins.enabled.posts
          ? {
              posts: {
                ...
                list: new Array(Number(options["posts.limit"]))
                  .fill(null)
                  .map((_) => ({
        ...
        ...(set.plugins.enabled.topics
          ? {
              topics: {
                ...
                list: new Array(Number(options["topics.limit"]) || 20)
                  .fill(null)
                  .map((_) => ({
          ...
          ? {
              stars: {
                repositories: [
                  ...
                  ...new Array(Number(options["stars.limit"]) - 1)
                    .fill(null)
                    .map((_, i) => ({
                      starredAt: faker.date.recent(),
                      node: {
              ...
              get stargazers() {
                ...
                for (let d = -14; d <= 0; d++) {
                  const date = new Date(Date.now() - d * 24 * 60 * 60 * 1000)
                    ...
                    .substring(0, 10);
                  dates.push(date);
                  result.total.dates[date] = total += result.increments.dates[
                    date
        ...
        ...(set.plugins.enabled.wakatime
          ? {
              get wakatime() {
                const stats = (array) => {
                  const elements = [];
                  let results = new Array(4 + faker.random.number(2))
                    .fill(null)
                    .map((_) => ({
                      name: array
                  ...
                  results.filter(({ name }) =>
                    elements.includes(name)
                      ? false
                      : (elements.push(name), true)
                  );
                  return results.sort((a, b) => b.percent - a.percent);
                };
                return {
                  sections: options["wakatime.sections"]
                    .split(",")
                    .map((x) => x.trim())
                    .filter((x) => x),
        ...
        ...(set.plugins.enabled.anilist
          ? {
              anilist: {
                ...
                get lists() {
                  const media = (type) => ({
                    ...
                    scores: {
                      ...
                      community: faker.random.number(100),
                    },
                    released: 100 + faker.random.number(1000),
                    artwork:
                      "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mOcOnfpfwAGfgLYttYINwAAAABJRU5ErkJggg==",
                  });
                  const sections = options["anilist.sections"]
                    .split(",")
                    .map((x) => x.trim())
                    .filter((x) => x);
                  const medias = options["anilist.medias"]
                    .split(",")
                    .map((x) => x.trim())
                    .filter((x) => x);
                  return {
                    ...(medias.includes("anime")
                      ? {
                          anime: {
                            ...(sections.includes("watching")
                              ? {
                                  watching: new Array(
                                    Number(options["anilist.limit"]) || 4
                                  )
                                    .fill(null)
                                    .map((_) => media("ANIME")),
                                }
                ...
                sections: options["anilist.sections"]
                  .split(",")
                  .map((x) => x.trim())
                  .filter((x) => x),
              },
        ...
        ...(set.plugins.enabled.activity
          ? {
              activity: {
                events: new Array(Number(options["activity.limit"]))
                  .fill(null)
                  .map(
                    (_) =>
                      [
                        ...
                        },
                        {
                          ...
                          title: faker.lorem.sentence(),
                          lines: {
                            added: faker.random.number(1000),
                            deleted: faker.random.number(1000),
                          },
                          files: { changed: faker.random.number(10) },
                        },
                        {
                          type: "wiki",
                          repo: `${faker.random.word()}/${faker.random.word()}`,
                        ...
                        },
                        {
                          ...
                          ref: {
                            ...
                            type: faker.random.arrayElement(["tag", "branch"]),
                          },
                        },
                        {
                          type: "ref/delete",
        ...
        ...(set.plugins.enabled.isocalendar
          ? {
              isocalendar: {
                streak: {
                  max: 30 + faker.random.number(20),
                  current: faker.random.number(30),
                },
