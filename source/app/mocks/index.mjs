...
export default async function ({ graphql, rest }) {
  ...
  if (mocked) return { graphql, rest };
  mocked = true;
  console.debug(`metrics/compute/mocks > mocking`);

  //Load mocks
  const __mocks = paths.join(
    paths.dirname(urls.fileURLToPath(import.meta.url))
  );
  const mock = async ({ directory, mocks }) => {
    for (const entry of await fs.readdir(directory)) {
      if ((await fs.lstat(paths.join(directory, entry))).isDirectory()) {
        if (!mocks[entry]) mocks[entry] = {};
        await mock({
          directory: paths.join(directory, entry),
          mocks: mocks[entry],
        });
      } else
        mocks[entry.replace(/[.]mjs$/, "")] = (
          await import(urls.pathToFileURL(paths.join(directory, entry)).href)
        ).default;
    }
    return mocks;
  };
  const mocks = await mock({
    directory: paths.join(__mocks, "api"),
    mocks: {},
  });
  ...
  //GraphQL API mocking
  {
    ...
    graphql = new Proxy(unmocked, {
      apply(target, that, args) {
        ...
        const [query] = args;
        const login =
          query.match(/login: "(?<login>.*?)"/)?.groups?.login ??
        ...
        for (const mocked of Object.keys(mocks.github.graphql))
          if (
            new RegExp(
              `^query ${mocked
                .replace(/([.]\w)/g, (_, g) =>
                  g.toLocaleUpperCase().substring(1)
                )
                .replace(/^(\w)/g, (_, g) => g.toLocaleUpperCase())} `
            ).test(query)
          )
  ...
  //Rest API mocking
  {
    ...
    const mocker = ({ path = "rest", mocks, mocked }) => {
      for (const [key, value] of Object.entries(mocks)) {
        console.debug(
          `metrics/compute/mocks > mocking rest api > mocking ${path}.${key}`
        );
        if (typeof value === "function") {
          unmocked[path] = value;
          mocked[key] = new Proxy(unmocked[path], {
            apply: value.bind(null, { faker }),
          });
        } else
          mocker({
            path: `${path}.${key}`,
