...
export default function ({
  ...
  if (/^https:..wakatime.com.api.v1.users.current.stats.*$/.test(url)) {
    ...
    if (/api_key=MOCKED_TOKEN/.test(url)) {
      console.debug(
        `metrics/compute/mocks > mocking wakatime api result > ${url}`
      );
      const stats = (array) => {
        const elements = [];
        let results = new Array(4 + faker.random.number(2))
          ...
          .map((_) => ({
            ...
            hours: faker.random.number(1000),
            minutes: faker.random.number(1000),
            name: array
              ? faker.random.arrayElement(array)
              : faker.random.words(2).replace(/ /g, "-").toLocaleLowerCase(),
            percent: 0,
            total_seconds: faker.random.number(1000000),
          }));
        results = results.filter(({ name }) =>
          elements.includes(name) ? false : (elements.push(name), true)
        );
        let percents = 100;
      ...
      return {
        ...
        data: {
          data: {
            ...
            daily_average_including_other_language: faker.random.number(
              12 * 60 * 60
            ),
            dependencies: stats(),
            editors: stats([
            ...
            total_seconds_including_other_language: faker.random.number(
              1000000000
            ),
          },
        },
