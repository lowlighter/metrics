...
export default function ({
  ...
  if (/^https:..api.twitter.com.*$/.test(url)) {
    //Get user profile
    if (
      /users.by.username/.test(url) &&
      options?.headers?.Authorization === "Bearer MOCKED_TOKEN"
    ) {
      console.debug(
        `metrics/compute/mocks > mocking twitter api result > ${url}`
      );
      const username =
        url.match(/username[/](?<username>.*?)[?]/)?.groups?.username ??
