...
export default function ({
  ...
  if (/^https:..graphql.anilist.co.*$/.test(url)) {
    ...
    const media = ({ type }) => ({
      title: {
        romaji: faker.lorem.words(),
        english: faker.lorem.words(),
        native: faker.lorem.words(),
      },
    ...
    if (/^query Statistics /.test(query)) {
      console.debug(
        `metrics/compute/mocks > mocking anilist api result > Statistics`
      );
      return {
        status: 200,
        data: {
    ...
    if (/^query FavoritesCharacters /.test(query)) {
      console.debug(
        `metrics/compute/mocks > mocking anilist api result > Favorites characters`
      );
      return {
        status: 200,
        data: {
          data: {
            User: {
              favourites: {
                characters: {
                  nodes: new Array(2 + faker.random.number(16))
                    .fill(null)
                    .map((_) => ({
                      name: {
                        full: faker.name.findName(),
                        native: faker.name.findName(),
                      },
                      image: { medium: null },
                    })),
    ...
    if (/^query Favorites /.test(query)) {
      console.debug(
        `metrics/compute/mocks > mocking anilist api result > Favorites`
      );
      const type = /anime[(]/.test(query)
        ? "ANIME"
        : /manga[(]/.test(query)
        ? "MANGA"
        : "OTHER";
    ...
    if (/^query Medias /.test(query)) {
      console.debug(
        `metrics/compute/mocks > mocking anilist api result > Medias`
      );
