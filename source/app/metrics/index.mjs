...
export default async function metrics(
  { login, q, dflags = [] },
  { graphql, rest, plugins, conf, die = false, verify = false, convert = null },
  { Plugins, Templates }
) {
  //Compute rendering
  try {
    ...
    if (
      ...
      !(template in conf.templates) ||
      (conf.settings.templates.enabled.length &&
        !conf.settings.templates.enabled.includes(template))
    )
    ...
    //Partial parts
    {
      data.partials = new Set([
        ...decodeURIComponent(q["config.order"] ?? "")
          .split(",")
          .map((x) => x.trim().toLocaleLowerCase())
          .filter((partial) => partials.includes(partial)),
        ...partials,
      ]);
      console.debug(
    ...
    await Plugins.base(
      { login, q, data, rest, graphql, plugins, queries, pending, imports },
      conf
    );
    await computer(
      { login, q, dflags },
      { conf, data, rest, graphql, plugins, queries, account: data.account },
      { pending, imports }
    );
    ...
    if (errors.length) {
      ...
      if (die) throw new Error(`An error occured during rendering, dying`);
      else
        console.warn(
    ...
    const { resized, mime } = await imports.svgresize(rendered, {
      paddings: q["config.padding"],
      convert,
    });
    rendered = resized;
    ...
    if (/svg/.test(mime)) {
      ...
      if (conf.settings?.optimize && !q.raw) {
        ...
        const svgo = new SVGO({
          full: true,
          plugins: [{ cleanupAttrs: true }, { inlineStyles: false }],
        });
