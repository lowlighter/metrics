...
export async function svgresize(svg, { paddings = ["6%"], convert } = {}) {
  ...
  if (!svgresize.browser) {
    svgresize.browser = await puppeteer.launch({
      headless: true,
    ...
    console.debug(
      `metrics/svgresize > started ${await svgresize.browser.version()}`
    );
  }
  //Format padding
  const [pw = 1, ph] = paddings
    .map((padding) => `${padding}`.substring(0, padding.length - 1))
    .map((value) => 1 + Number(value) / 100);
  const padding = { width: pw, height: ph ?? pw };
  console.debug(
    `metrics/svgresize > padding width*${padding.width}, height*${padding.height}`
  );
  ...
  let { resized, width, height } = await page.evaluate(async (padding) => {
    ...
    const animated = !document
      .querySelector("svg")
      .classList.contains("no-animations");
    if (animated) document.querySelector("svg").classList.add("no-animations");
    //Get bounds and resize
    let { y: height, width } = document
      .querySelector("svg #metrics-end")
    ...
    //Result
    return {
      resized: new XMLSerializer().serializeToString(
