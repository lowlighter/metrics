...
export default async function ({
  ...
  if (fs.existsSync(__settings)) {
    if (nosettings)
      logger(
        `metrics/setup > load settings.json > skipped because no settings is enabled`
      );
    else {
      conf.settings = JSON.parse(`${await fs.promises.readFile(__settings)}`);
      logger(`metrics/setup > load settings.json > success`);
    }
  } else logger(`metrics/setup > load settings.json > (missing)`);
  ...
  conf.settings.plugins.base = {
    parts: ["header", "activity", "community", "repositories", "metadata"],
  };
  ...
  if (
    typeof conf.settings.community.templates === "string" &&
    conf.settings.community.templates.length
  ) {
  ...
  if (
    Array.isArray(conf.settings.community.templates) &&
    conf.settings.community.templates.length
  ) {
    //Clean remote repository
    logger(
      `metrics/setup > ${conf.settings.community.templates.length} community templates to install`
    );
    await fs.promises.rmdir(path.join(__templates, ".community"), {
      recursive: true,
    });
    ...
    for (const template of conf.settings.community.templates) {
      try {
        ...
        const { repo, branch, name, trust = false } = template.match(
          /^(?<repo>[\s\S]+?)@(?<branch>[\s\S]+?):(?<name>[\s\S]+?)(?<trust>[+]trust)?$/
        )?.groups;
        const command = `git clone --single-branch --branch ${branch} https://github.com/${repo}.git ${path.join(
          __templates,
          ".community"
        )}`;
        ...
        await fs.promises.rmdir(path.join(__templates, `@${name}`), {
          recursive: true,
        });
        await fs.promises.rename(
          path.join(__templates, ".community/source/templates", name),
          path.join(__templates, `@${name}`)
        );
        //JavaScript file
        if (trust)
          logger(
            `metrics/setup > keeping @${name}/template.mjs (unsafe mode is enabled)`
          );
        else if (
          fs.existsSync(path.join(__templates, `@${name}`, "template.mjs"))
        ) {
          logger(`metrics/setup > removing @${name}/template.mjs`);
          await fs.promises.unlink(
            path.join(__templates, `@${name}`, "template.mjs")
          );
  ...
  } else logger(`metrics/setup > no community templates to install`);
  ...
  for (const name of await fs.promises.readdir(__templates)) {
    ...
    if (
      !(await fs.promises.lstat(directory)).isDirectory() ||
      !fs.existsSync(path.join(directory, "partials/_.json"))
    )
      continue;
    logger(`metrics/setup > load template [${name}]`);
    //Cache templates files
    const files = ["image.svg", "style.css", "fonts.css"].map((file) =>
      path.join(
        __templates,
        fs.existsSync(path.join(directory, file)) ? name : "classic",
        file
      )
    );
    const [image, style, fonts] = await Promise.all(
      files.map(async (file) => `${await fs.promises.readFile(file)}`)
    );
    const partials = JSON.parse(
      `${await fs.promises.readFile(path.join(directory, "partials/_.json"))}`
    );
    conf.templates[name] = {
      image,
      style,
    ...
    Templates[name] = await (async () => {
      ...
      return (
        await import(
          url.pathToFileURL(fs.existsSync(template) ? template : fallback).href
        )
      ).default;
    })();
    ...
    if (conf.settings.debug) {
      Object.defineProperty(conf.templates, name, {
        get() {
          ...
          const [image, style, fonts] = files.map(
            (file) => `${fs.readFileSync(file)}`
          );
          const partials = JSON.parse(
            `${fs.readFileSync(path.join(directory, "partials/_.json"))}`
          );
  ...
  for (const name of await fs.promises.readdir(__plugins)) {
    ...
    if (!(await fs.promises.lstat(directory)).isDirectory()) continue;
    //Cache plugins scripts
    logger(`metrics/setup > load plugin [${name}]`);
    Plugins[name] = (
      await import(url.pathToFileURL(path.join(directory, "index.mjs")).href)
    ).default;
    ...
    if (fs.existsSync(__queries)) {
      ...
      for (const file of await fs.promises.readdir(__queries)) {
        ...
        queries[`_${query}`] = `${await fs.promises.readFile(
          path.join(__queries, file)
        )}`;
        ...
        if (conf.settings.debug) {
          Object.defineProperty(queries, `_${query}`, {
            get() {
              ...
              logger(
                `metrics/setup > reload query [${name}/${query}] > success`
              );
              return raw;
            },
          });
