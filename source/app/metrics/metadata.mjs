...
export default async function metadata({ log = true } = {}) {
  ...
  for (const name of await fs.promises.readdir(__templates)) {
    ...
    Templates[name] = await metadata.template({
      __templates,
      name,
      plugins,
      logger,
...
metadata.plugin = async function ({ __plugins, name, logger }) {
  try {
    ...
    const raw = `${await fs.promises.readFile(
      path.join(__plugins, name, "metadata.yml"),
      "utf-8"
    )}`;
      ...
      meta.inputs = function (
        ...
        if (account !== "bypass") {
          if (!meta.supports?.includes(account))
            throw {
              error: {
                message: `Not supported for: ${account}`,
                instance: new Error(),
              },
            };
          if (q.repo && !meta.supports?.includes("repository"))
            throw {
              error: {
                message: `Not supported for: ${account} repositories`,
                instance: new Error(),
              },
            };
        }
        ...
        const result = Object.fromEntries(
          Object.entries(inputs).map(
            ([key, { type, format, default: defaulted, min, max, values }]) => [
              //Format key
              ...
              ((defaulted) => {
                ...
                switch (type) {
                  ...
                  case "number": {
                    value = Number(value);
                    if (!Number.isFinite(value)) value = defaulted;
                  ...
                  case "array": {
                    try {
                      ...
                      logger(
                        `metrics/inputs > failed to decode uri : ${value}`
                      );
                      value = defaulted;
                    }
                    const separators = {
                    ...
                    const separator =
                      separators[
                        [format].flat().filter((s) => s in separators)[0]
                      ] ?? ",";
                    return value
                      .split(separator)
                      .map((v) => v.trim().toLocaleLowerCase())
                      .filter((v) =>
                        Array.isArray(values) ? values.includes(v) : true
                      )
                      .filter((v) => v);
                  }
                  //String
                  case "string": {
                    value = `${value}`.trim();
                    if (user) {
                      if (value === ".user.login") return user.login;
                      if (value === ".user.twitter")
                        return user.twitterUsername;
                      if (value === ".user.website") return user.websiteUrl;
                    }
                  ...
                  case "json": {
                    try {
                      ...
                      logger(
                        `metrics/inputs > failed to parse json : ${value}`
                      );
      ...
      raw
        ...
        .map((x) => {
          const input =
            x.match(
              new RegExp(
                `^\\s*(?<input>${Object.keys(inputs).join("|")}):`,
                "m"
              )
            )?.groups?.input ?? null;
          if (input)
            comments[input] = x.match(
              new RegExp(
                `(?<comment>[\\s\\S]*?)(?=(?:${Object.keys(inputs)
            ...
            descriptor: yaml.dump(
              {
                [key]: Object.fromEntries(
                  Object.entries(value).filter(([key]) =>
                    ["description", "default", "required"].includes(key)
                  )
                ),
              },
    ...
    //Readme metadata
    {
      //Extract demos
      const raw = `${await fs.promises.readFile(
        path.join(__plugins, name, "README.md"),
        "utf-8"
      )}`;
      const demo =
        raw
          .match(/(?<demo><table>[\s\S]*?<[/]table>)/)
          ?.groups?.demo?.replace(/<[/]?(?:table|tr)>/g, "")
          ?.trim() ?? "<td></td>";
...
metadata.template = async function ({ __templates, name, plugins, logger }) {
  try {
    //Load meta descriptor
    const raw = `${await fs.promises.readFile(
      path.join(__templates, name, "README.md"),
      "utf-8"
    )}`;
    ...
    if (
      fs.existsSync(partials) &&
      (await fs.promises.lstat(partials)).isDirectory()
    ) {
      for (let plugin of await fs.promises.readdir(partials)) {
        plugin =
          plugin.match(/(?<plugin>^[\s\S]+(?=[.]ejs$))/)?.groups?.plugin ??
          null;
        if (plugin in compatibility) compatibility[plugin] = true;
      }
    }
