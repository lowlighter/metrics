...
info.section = (left = "", right = " ") =>
  info(`\x1b[36m${left}\x1b[0m`, right);
info.group = ({ metadata, name, inputs }) => {
  info.section(
    metadata.plugins[name]?.name
      ?.match(/(?<section>[\w\s]+)/i)
      ?.groups?.section?.trim(),
    " "
  );
  for (const [input, value] of Object.entries(inputs))
    info(metadata.plugins[name]?.inputs[input]?.description ?? input, value, {
      token: metadata.plugins[name]?.inputs[input]?.type === "token",
    });
};
...
try {
  ...
  if (
    ...
    if (
      /\[Skip GitHub Action\]/.test(github.context.payload.head_commit.message)
    ) {
  ...
  const { conf, Plugins, Templates } = await setup({
    log: false,
    nosettings: true,
    community: { templates: core.getInput("setup_community_templates") },
  });
  const { metadata } = conf;
  info("Setup", "complete");
  ...
  if (!token)
    throw new Error(
      "You must provide a valid GitHub token to gather your metrics"
    );
  conf.settings.token = token;
  const api = {};
  api.graphql = octokit.graphql.defaults({
  ...
  if (!dryrun) {
    ...
    committer.branch =
      _branch || github.context.ref.replace(/^refs[/]heads[/]/, "");
    info("Committer token", committer.token, { token: true });
    if (!committer.token)
      throw new Error(
        "You must provide a valid GitHub token to commit your metrics"
      );
    ...
    try {
      info(
        "Committer account",
        (await committer.rest.users.getAuthenticated()).data.login
      );
    } catch {
  ...
  info(
    "Plugin errors",
    die ? "(exit with error)" : "(displayed in generated SVG)"
  );
  Object.assign(q, config);

  //Base content
  info.break();
  const { base: parts, ...base } = metadata.plugins.base.inputs.action({
    core,
  });
  ...
  //Additional plugins
  const plugins = {};
  for (const name of Object.keys(Plugins).filter(
    (key) => !["base", "core"].includes(key)
  )) {
    //Parse inputs
    const { [name]: enabled, ...inputs } = metadata.plugins[
      name
    ].inputs.action({ core });
  ...
  const { rendered } = await metrics(
    { login: user, q, dflags },
    { graphql, rest, plugins, conf, die, verify },
    { Plugins, Templates }
  );
