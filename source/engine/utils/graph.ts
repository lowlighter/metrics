...
export class Graph {
    ...
    for (const [name, {color = this.color(name), lines = true, areas = true, points = true, texts = true, data}] of Object.entries(datalist)) {
      ...
      if (texts) {
        svg.append("g")
          ...
          .text(d => d[2] ? (d[2] instanceof Date ? d[2].toISOString() : d[2]) : "")
          .attr("fill", this.config.axis.color)
      }
    }
  ...
  /** Create a pie graph */
  static pie(datalist:Record<PropertyKey, {color?:string, data:number}>, { width = this.config.width, height = this.config.height, ...options}:Pick<options, "width"|"height"|"legend"> = {}) {
    //Generate SVG
    const margin = this.config.margin
    ...
    svg.append("g")
      ...
      .data(D)
      .join("text")
      .attr("transform", d => `translate(${labels.centroid(d as d3arg)})`)
      .selectAll("tspan")
      .data(d => {
  ...
  /** Create a diff graph */
  static diff(datalist:Record<PropertyKey, {color?:string, data:{date:Date, added:number, deleted:number, changed:number}[]}>, {width = this.config.width, height = this.config.height} = {}) {
    // Create SVG
    ...
    // Graph areas
    for (const {type, sign, fill} of [{type: "added", sign: +1, fill: "rgb(63, 185, 80)"}, {type: "deleted", sign: -1, fill: "rgb(218, 54, 51)"}] as const) {
