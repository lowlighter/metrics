...
import { fromZodError } from "y/zod-validation-error@1.5.0"
import { MetricsError } from "@utils/errors.ts"
...
/** Parse input (sync) */
function _parse<T extends Validator>(input: T, vars: Record<PropertyKey, unknown>) {
...
/** Parse input (async) */
async function _parseAsync<T extends Validator>(input: T, vars: Record<PropertyKey, unknown>) {
...
/** Parse input */
export function parse<T extends Validator>(input: T, vars: Record<PropertyKey, unknown>, options: { sync: true }): is.infer<T>
export function parse<T extends Validator>(input: T, vars: Record<PropertyKey, unknown>, options?: { sync?: false }): Promise<is.infer<T>>
export function parse<T extends Validator>(input: T, vars: Record<PropertyKey, unknown>, { sync = false } = {}) {
