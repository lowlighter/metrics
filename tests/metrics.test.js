...
action.defaults = Object.fromEntries(
  Object.entries(action.inputs).map(([key, { default: value }]) => [key, value])
);
action.input = (vars) =>
  Object.fromEntries(
    [
      ...Object.entries(action.defaults),
      ...Object.entries(vars),
    ].map(([key, value]) => [`INPUT_${key.toLocaleUpperCase()}`, value])
  );
action.run = async (vars) =>
  await new Promise((solve, reject) => {
    ...
    const env = {
      ...process.env,
      ...action.input(vars),
      GITHUB_REPOSITORY: "lowlighter/metrics",
    };
    const child = processes.spawn("node", ["source/app/action/index.mjs"], {
      env,
    });
    child.stdout.on("data", (data) => (stdout += data));
    child.stderr.on("data", (data) => (stderr += data));
    child.on("close", (code) => {
      if (code === 0) return solve(true);
...
web.run = async (vars) =>
  (
    await axios(
      `http://localhost:3000/lowlighter?${new url.URLSearchParams(
        Object.fromEntries(
...
web.start = async () =>
  new Promise((solve) => {
    ...
    web.instance = processes.spawn("node", ["source/app/web/index.mjs"], {
      env: { ...process.env, USE_MOCKED_DATA: true, NO_SETTINGS: true },
    });
    web.instance.stdout.on(
      "data",
      (data) => (
        (stdout += data), /Server ready !/.test(stdout) ? solve() : null
      )
    );
...
placeholder.run = async (vars) => {
  ...
  const enabled = Object.fromEntries(
    Object.entries(vars).filter(([key]) => /^plugin_[a-z]+$/.test(key))
  );
  const config = Object.fromEntries(
    Object.entries(options).filter(([key]) => /^config[.]/.test(key))
  );
  const base = Object.fromEntries(
    Object.entries(options).filter(([key]) => /^base[.]/.test(key))
  );
  return (
    typeof (await placeholder({
...
beforeAll(async (done) => {
  ...
  await fs.promises.rmdir(
    path.join(__dirname, "../source/templates/@classic"),
    { recursive: true }
  );
  //Start web instance
  await web.start();
...
for (const name in metadata.plugins) {
  const cases = yaml
    .load(
      fs.readFileSync(
        path.join(__dirname, "../source/plugins", name, "tests.yml"),
        "utf8"
      )
    )
...
describe("GitHub Action", () =>
  describe.each([
    ...
    for (const [name, input, { skip = [], modes = [] } = {}] of tests)
      if (
        skip.includes(template) ||
        (modes.length && !modes.includes("action"))
      )
        test.skip(name, () => null);
      else
        test(name, async () =>
...
describe("Web instance (placeholder)", () =>
  describe.each([
    ...
    for (const [name, input, { skip = [], modes = [] } = {}] of tests)
      if (
        skip.includes(template) ||
        (modes.length && !modes.includes("placeholder"))
      )
        test.skip(name, () => null);
      else
        test(name, async () =>
          expect(
            await placeholder.run({ template, base: 0, ...query, ...input })
          ).toBe(true)
        );
  }));
