...
//Paths
const __metrics = paths.join(
  paths.dirname(url.fileURLToPath(import.meta.url)),
  ".."
);
...
//Rendering
await update({
  source: paths.join(__readme, "README.md"),
  output: "README.md",
  options: { root: __readme },
});
await update({
  source: paths.join(__readme, "partials/documentation/plugins.md"),
  output: "source/plugins/README.md",
});
await update({
  source: paths.join(__readme, "partials/documentation/templates.md"),
  output: "source/templates/README.md",
});
await update({
  source: paths.join(__action, "action.yml"),
  output: "action.yml",
});
await update({
  source: paths.join(__web, "settings.example.json"),
  output: "settings.example.json",
});
